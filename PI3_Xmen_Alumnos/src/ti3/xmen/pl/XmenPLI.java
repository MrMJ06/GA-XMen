...
public class XmenPLI {
	...
	public static String getConstraints(String s) {
		...
		for (int j = 0; j < numMutantes; j++) {
			for (int i = 0; i < numXmen; i++) {
				if (xm.get(i).isLider() == false && mt.get(j).isLider()
						|| xm.get(i).isLider() && mt.get(j).isLider() == false) {
					r += "x" + i + "" + j + " = 0;\n";
				}
			}
		}
		...
		for (int j = 0; j < numMutantes; j++) {
			for (int i = 0; i < numXmen; i++) {
				if (getTipo(xm.get(i)).contains(TipoMutante.FUEGO) 
								&& getTipo(mt.get(j)).contains(TipoMutante.HIELO)
						|| getTipo(mt.get(j)).contains(TipoMutante.FUEGO)
								&& getTipo(xm.get(i)).contains(TipoMutante.HIELO)
